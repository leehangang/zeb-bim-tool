...
**RAG 색인 = 15개 법령·고시·공고 원문 (1,140청크)**
...
| 01 | GR 가이드라인 (79쪽) | 2026-03 |
| 02 | GR 기술요소 세부기준 | — |
| 09 | 영유아보육법 시행규칙 (제382호) | 2026-05-12 |
...
> 🔧 **2026-07 해결** — `01`·`02`·`09`는 "이미지 스캔본이라 추출 불가"로 제외돼 있었으나,
> 실제로는 **페이지마다 `.txt`가 동봉된 번들 포맷**(`manifest.json` + 페이지 이미지 + 텍스트)이었습니다.
> 인덱서가 `.jpeg`만 보고 "텍스트 없음"이라 단정해 버리고 있었을 뿐입니다 — **+114쪽 · 13만자 회수**.
> `03`은 `12`(ZEB 인증규칙)와 **동일 문서**(정규화 텍스트 11,000자 완전 일치)라 중복 제외했습니다.
>
> ⚠️ **아직 없는 원문** — 별표1(자립률 산식·대지외 보정계수) · 별표2(등급표) · 별표1의2(BEMS 13항목)는
> 이 규칙이 아니라 **ZEB 인증기준 공동고시**(국토부 제2024-893호 / 산업부 제2024-208호)에 있습니다.
> 인증규칙 §8③이 *"국토교통부장관과 기후에너지환경부장관이 정하여 공동으로 고시한다"* 고 위임합니다.
> 확보하면 보정계수가 `확인필요_원문미확보` → `확인됨_원문대조`로 승격됩니다.
...
저장소에 이미 빌드된 인덱스(`data/chroma_db.zip`, 15개 원문 · 1,140청크)가 들어 있어
