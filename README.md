...
Revit BIM 모델을 업로드하면 11개 GR 기술요소 자동 평가 → 보강 우선순위 + Max Cost + 보조금 + 회수기간을 통합 산출하는 **7-모드 플랫폼**.
...
|---|---|---|
| 근거 | ZEB 인증기준 고시 (제1·2·3호) | GR 고시 §9 + 2026 공공 GR 2.0 가이드라인 《표1》 |
| 판정 | 자립률/1차에너지 + BEMS → **+등급~5등급** | 100점 → **등급 없음** (고득점 순 선정 랭킹 점수) |
...
| 🏢 **BIM 진단 + ROI** | Revit BIM JSON | ZEB 등급 · GR 정량평가 점수 · 보강 우선순위 | 불필요 |
| 💬 **정책 Q&A (RAG)** | 자연어 질문 | 근거 조항 인용 답변 | 필요 |
| 💰 **ROI 시뮬레이션** | 자연어 ("연면적 1,200㎡, ZEB 5등급") | Max Cost / 보조금 / 회수기간 | 필요 |
| 📋 **사업 신청 인테이크** | 챗봇 대화 | 신청서 마크다운 초안 | 필요 |
| 📐 **근거·출처** | — | 파라미터 출처 · 확인필요 · 등급 민감도 · 데이터 처리 고지 | 불필요 |
| 📖 **용어사전** | — | 세 제도 구분 · 헷갈리는 지점 · 용어 13개 | 불필요 |
...
### 📖 용어사전 — 우리가 틀렸던 혼동까지 공개

자립률·1차에너지소요량·성능개선비율·정량평가표·B-C가 한 화면에 나옵니다. 심사위원·공무원이
이 말을 모르면 숫자를 봐도 판단할 수 없습니다. 정의뿐 아니라 **함정과 우리 도구의 한계**를 같이 씁니다.

**핵심은 제도 구분입니다** — 이름이 비슷한 세 제도가 섞이기 쉽고, 실제로 **우리가 섞었습니다**:

| 제도 | 근거 | 결과 |
|---|---|---|
| ZEB 인증 | 녹색건축법 **§17** | 등급 (+등급·1~5등급) |
| 녹색건축 인증 (G-SEED) | **§16** | 등급 (최우수·우수·우량·일반) — **우리는 평가 안 함** |
| 그린리모델링 지원사업 | **§27** | **등급 없음** — 자격 + 선정 랭킹 점수 |

「⚠️ 헷갈리는 지점」 탭은 **우리 사고 기록**입니다 — 없는 등급을 만든 것, G-SEED 용어를
가짜 등급에 붙인 것, 두 트랙의 분모를 섞을 뻔한 것, 절감률을 단순합산한 것.
같은 실수를 반복하지 않기 위해, 그리고 우리 숫자를 인용하는 사람이 조심할 지점을 알도록 남깁니다.
...
저장소에 이미 빌드된 인덱스(`data/chroma_db.zip`, 12개 원문 · 974청크)가 들어 있어
...
| GR 정량평가 (현재) | **24/100점** | 선정 랭킹 점수 — 등급 아님 · 채점가능최대 92 |
| 전체 보강 비용 | 5.31억 (Max Cost) | 11개 항목 전체 |
| GR 정량평가 (보강 후) | **75/100점** | +52점 (11개 전체 보강 시) |
