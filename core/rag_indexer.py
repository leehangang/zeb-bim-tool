...
def chunk_text(
    ...
    ── 조(條) 단위 분할을 시도했다가 되돌린 기록 (2026-07-16) ──
    "제N조(제목)" 경계로 먼저 쪼개면 조문 인용이 정확해질 것 같았다.
    실제로 녹색건축법 시행규칙 §6이 §5에 묻히는 문제가 있었고, 분할하니 단독 청크가 됐다.
    그런데 **측정하니 개악이었다** (질문 10개, 답 문구가 인용 청크에 있는지):
        고정 길이  : top1 60% · top3 **90%**   (1,300청크)
        조 단위    : top1 50% · top3 **60%**   (1,936청크)
    문서 적중(top1 73%/top3 86%)은 양쪽 동일. 즉 조 분할은 이득 없이 인용만 망쳤다.
    이유 — 조가 짧으면 top-k에 담기는 본문 총량이 줄어 답이 빠진다. 긴 청크가 답을
    품을 확률이 높다. 청크 수만 49% 늘어 저장·배포 용량도 커진다.
    → 되돌린다. 조문 단위가 옳아 보인다는 직관보다 측정이 우선이다.
       (Mode 01이 "조항 단위로 색인"이라 말하던 문구도 사실이 아니므로 함께 정정)
