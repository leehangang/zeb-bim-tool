...
· 기상파일은 추풍령(471350) TMYx 하나뿐이다 — 중부2 최근접이지 대상지 실측이 아니다.
...
def write_idf(
    ...
    weather_hint: str = "중부2 기후대 — 추풍령 471350 최근접 (.epw 필요)",
) -> dict:
    """
...
!-  EnergyPlus는 GR 센터 지정 프로그램이고 '에너지 요구량' 기준도 허용됩니다
!-  (2026 민간 GR 공고 p.3·p.16 각주) → IdealLoads 부하로도 지표는 성립합니다.
!-  ⚠️ 그러나 같은 각주가 용도프로필·기상데이터를 ZEB 운영규정 별표2·별표6으로
!-     준용하라고 합니다. 이 파일은 둘 다 안 지킵니다(표준가정 프로필 + 추풍령 .epw).
!-     → 신청에 쓰려면 별표2·별표6 입력으로 다시 돌려야 합니다.
!-  ZEB 인증은 또 별개입니다 — 계산법·소요량·급탕·신재생이 전부 다릅니다.
!-  ==========================================================
