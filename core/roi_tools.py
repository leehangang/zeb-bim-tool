        ...
        "properties": {
            ...
            "annual_energy_saving_won_per_m2": {
                ...
                "description": (
                    ...
                    "기본값은 한전 기본공급약관(2026.7.1 시행) 원문 단가로 산출한다 — "
                    "(1차E 절감 ÷ 전력환산계수 2.75) × 세후 실효단가 140.9원/kWh. "
                    "⚠️ 단가는 원문 근거가 있으나 곱해지는 **절감 kWh**(base 200·요소별 절감률)는 "
                    "ECO2 정식 해석 전이라 추정치다. 실측 고지서 값이 있으면 반드시 그것을 넣을 것."
                ),
            ...
            "cap_rate": {
                "type": "number",
                "description": (
                    "환원율(Cap rate, 소수). 수익환원법 자산가치 산정용 "
                    "(ΔNOI ÷ 환원율). 낮을수록 자산가치 큼. 기본 0.05(5%). "
                    "참고: 성수동 업무시설 4.3%."
                ),
                "default": 0.05,
            },
...
def _dispatch_calculate_zeb_roi(args: dict) -> dict:
    ...
    building_info = {
        ...
        "cap_rate": float(args.get("cap_rate", 0.05)),
    }
    ...
    return {
        ...
        "자산가치_수익환원": (
            {
                "방식": result["asset_value"]["방식"],
                "환원율": result["asset_value"]["환원율"],
                "연_NOI_증가_원": result["asset_value"]["연_NOI_증가_원"],
                "자산가치_상승_원": result["asset_value"]["자산가치_상승_원"],
                "자부담_대비_pct": result["asset_value"].get("자부담_대비_pct"),
                "자산_순증_원": result["asset_value"].get("자산_순증_원"),
            }
            if result.get("asset_value") else None
        ),
...
3-1. 자산가치는 두 관점으로 구분해 설명하세요:
   (a) 현금흐름 관점 = NPV/IRR/B-C (분석기간 동안 절감액의 할인가치)
   (b) 자산가치 관점 = 수익환원법 자산가치 상승 (ΔNOI ÷ 환원율, 영구 환원)
   이 둘은 같은 에너지 절감을 다른 방식으로 본 것이므로 절대 더하지 마세요(이중계산).
   수익환원 자산가치는 용적률 완화 증축 가정이 없어도 적용되며, 환원율을 반드시 명시하세요.
   용적률_완화 자산가치는 증축이 실제로 계획될 때만 유효한 '조건부' 항목으로 분리 제시하세요.
