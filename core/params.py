...
def zeb_fee_refund_rate(grade: str, is_mandatory: bool,
    ...
    🔴 2026년 지금 신청하면 **0이다.** 예전엔 ②를 안 보고 30%를 편익에 넣고 있었다 —
       대상 건물 ROI를 78만원 부풀리는 방향이었다. 우리에게 유리한 쪽 오류가 더 위험하다.
...
def annual_saving_per_m2(reduction_primary_kwh_m2: Optional[float] = None) -> float:
    ...
    대상 건물 기준 100.9 ÷ 2.75 × 140.9 ≈ **5,172원/㎡** — 옛 매직넘버는 **1.91배 과대**였다.
    (연간 1,238만원 → 647만원. 방향은 KEEI 실측연구 '예측의 60%'와 독립적으로 일치한다.)

    Args:
        reduction_primary_kwh_m2: 1차에너지 절감량. None이면 params의 대상 건물 기본값.

    ⚠️ 남은 가정 — **전량 전기**로 본다. 대상 건물의 실제 연료 구성(전기/가스)은 미확인이며,
       대상 시도 노유자시설 전기 비중은 82.2%다(건물에너지사용량통계 2024). 가스분이 있으면
