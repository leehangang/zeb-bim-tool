...
# ====================================================================
# 06 에너지절약설계기준 [별표1] 지역별 건축물 부위의 열관류율표 (W/㎡·K)
# ====================================================================
# 출처: 국토교통부고시 제2025-738호 [별표1] — **원문 PDF 직접 파싱으로 검증**
#       (data/policy_docs/06_에너지절약설계기준.pdf p.19)
#
# 🔴 2026-07 정정 — 기존 값은 **공동주택(주거) 기준**을 모든 용도에 쓰고 있었다.
#    별표1은 거실 외벽·창을 **"공동주택" / "공동주택 외"로 나눈다**:
#        중부2 외벽 외기직접:  공동주택 0.170  /  공동주택 외 0.240
#        중부2 창   외기직접:  공동주택 1.000  /  공동주택 외 1.500
#    대상 건물(어린이집=비주거)에 공동주택 값을 적용해 **실제보다 엄격하게 채점**하고 있었다.
#    기존 주석에도 "실제 06 PDF [별표1] 값과 비교 검증 후 사용 권장"이라 적혀 있었으나
#    검증되지 않은 채 돌고 있었다. 팀 학습문서 v4 p.10이 이 불일치를 짚어줘 확인했다.
#
# 구조: U_VALUE_LIMITS[지역][부위_직접|간접] → 비주거(공동주택 외) 기준
#       주거(공동주택)는 U_VALUE_LIMITS_RES 사용. is_residential로 고른다.
# --------------------------------------------------------------------

# 비주거 = 별표1 "공동주택 외" (대상 건물 등 대부분의 공공건축물)
U_VALUE_LIMITS = {
    "중부1": {
        "외벽_직접": 0.170, "외벽_간접": 0.240,
        "지붕_직접": 0.150, "지붕_간접": 0.210,
        "바닥_직접": 0.150, "바닥_간접": 0.210,   # 바닥난방인 경우
        "창_직접": 1.300, "창_간접": 1.600,
        "문_직접": 1.500, "문_간접": 1.900,
    },
    "중부2": {
        "외벽_직접": 0.240, "외벽_간접": 0.340,
        "지붕_직접": 0.180, "지붕_간접": 0.260,
        "바닥_직접": 0.170, "바닥_간접": 0.240,
        "창_직접": 1.500, "창_간접": 1.900,
        "문_직접": 1.500, "문_간접": 1.900,
    },
    "남부": {
        "외벽_직접": 0.320, "외벽_간접": 0.450,
        "지붕_직접": 0.250, "지붕_간접": 0.350,
        "바닥_직접": 0.220, "바닥_간접": 0.310,
        "창_직접": 1.800, "창_간접": 2.200,
        "문_직접": 1.500, "문_간접": 1.900,
    },
    "제주": {
        "외벽_직접": 0.410, "외벽_간접": 0.560,
        "지붕_직접": 0.290, "지붕_간접": 0.350,   # 별표1 제주 지붕 간접
        "바닥_직접": 0.290, "바닥_간접": 0.410,
        "창_직접": 2.200, "창_간접": 2.800,
        "문_직접": 1.500, "문_간접": 1.900,
    },
}

# 주거 = 별표1 "공동주택" (외벽·창만 별도 기준. 지붕·바닥은 용도 구분 없음)
U_VALUE_LIMITS_RES = {
    "중부1": {**U_VALUE_LIMITS["중부1"], "외벽_직접": 0.150, "외벽_간접": 0.210,
              "창_직접": 0.900, "창_간접": 1.300},
    "중부2": {**U_VALUE_LIMITS["중부2"], "외벽_직접": 0.170, "외벽_간접": 0.240,
              "창_직접": 1.000, "창_간접": 1.500},
    "남부":  {**U_VALUE_LIMITS["남부"],  "외벽_직접": 0.220, "외벽_간접": 0.310,
              "창_직접": 1.200, "창_간접": 1.700},
    "제주":  {**U_VALUE_LIMITS["제주"],  "외벽_직접": 0.290, "외벽_간접": 0.410,
              "창_직접": 1.600, "창_간접": 2.000},
}


def u_limits(region: str, is_residential: bool = False) -> dict:
    """
    지역·용도별 열관류율 기준 (별표1).

    ⚠️ 별표1은 거실 외벽·창을 '공동주택'과 '공동주택 외'로 나눈다.
       주거(공동주택)가 더 엄격하므로 비주거에 주거 기준을 쓰면 과소평가된다.
       주거 판정은 core.zeb_evaluator.RESIDENTIAL_USES를 따른다(별표2 주5 — 기숙사 제외).
    """
    table = U_VALUE_LIMITS_RES if is_residential else U_VALUE_LIMITS
    if region not in table:
        raise KeyError(f"알 수 없는 지역: {region} (가능: {list(table)})")
    return table[region]
...
def map_to_gr_elements(bim: dict) -> dict:
    ...
    # 고성능 판정: 외기 직접 면 + u_value <= 별표1 기준
    # ⚠️ 별표1은 창 기준을 주거/비주거로 나눈다 — 용도를 반영해야 한다.
    region = bim.get("region", "중부2")
    from core.zeb_evaluator import RESIDENTIAL_USES, detect_building_use
    _is_res = detect_building_use(bim) in RESIDENTIAL_USES
    win_u_limit = u_limits(region, _is_res)["창_직접"]
    high_perf_win = sum(
...
def check_u_value(
    ...
    Args:
        ...
        is_residential: 주거(공동주택)면 True — 별표1은 외벽·창을 주거/비주거로 나눈다.
                        기본은 비주거(공동주택 외). 우리 케이스 대부분이 공공 비주거다.
    ...
    try:
        table = u_limits(region, is_residential)
    except KeyError:
        table = {}
    if key not in table:
        return {
...
# gr_mapping 키 → 사람이 읽는 라벨.
#
# 🔴 왜 여기 있나: 이 표가 예전엔 generate_diagnosis_report() **안에** 있었다.
#    그래서 PDF(core/pdf_report.py)는 이걸 못 쓰고 `key.split("_")[1]`로 키를 쪼개 썼는데,
#    키는 식별자라 가운뎃점·괄호·공백이 없다 → PDF에만 '바닥단열난방', '신재생태양광',
#    '고효율냉난방'으로 나왔다. 글자가 깨진 게 아니라 **애초에 라벨이 아니었다.**
#    마크다운과 PDF가 같은 표를 보게 단일 소스로 뺀다.
GR_ELEMENT_LABELS = [
    ("1", "1_고성능창호", "고성능창호"),
    ("2", "2_고기밀성단열문", "고기밀성 단열문"),
    ("3", "3_외벽단열보강", "외벽 단열보강"),
    ("4", "4_바닥단열난방", "바닥단열·난방"),
    ("5", "5_쿨루프", "쿨루프"),
    ("6", "6_폐열회수환기", "폐열회수환기"),
    ...
    ("8", "8_고효율보일러", "고효율 보일러"),
    ("9", "9_고효율LED", "고효율 LED"),
    ("10", "10_신재생태양광", "신재생(태양광)"),
    ("6-나", "6-나_신재생태양열", "신재생(태양열)"),
    ("11", "11_BEMS", "BEMS"),
]


def gr_label(key: str) -> str:
    """gr_mapping 키 → 사람이 읽는 라벨. 모르는 키는 키를 그대로 돌려준다."""
    for _, k, label in GR_ELEMENT_LABELS:
        if k == key:
            return label
...
def generate_diagnosis_report(
    ...
    # 단일 소스(GR_ELEMENT_LABELS)를 쓴다 — PDF도 같은 표를 본다.
    # (예전엔 여기 손으로 적어둬서 태양열 행이 마크다운에만 빠져 있었다.)
    items = [
        (num, label, gr_mapping[key])
        for num, key, label in GR_ELEMENT_LABELS
        if key in gr_mapping
    ]
