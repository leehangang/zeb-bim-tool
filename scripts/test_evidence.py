...
if not _gaz.exists():
    ...
    # 2026-07-16: '운영세칙 미확보'였던 게 풀렸다. 「인증 제도 운영규정」[별표2] 용도프로필이
    # 그 운영세칙이고, law.go.kr에 무료 공개돼 있었다 — '비공개'가 아니라 '안 찾아본' 것이었다.
    # 다만 찾고 나서 더 나쁜 사실이 드러났다: 23개 용도에 어린이집·노유자시설이 없고,
    # 보정계수는 건물이 아니라 **공간(zone) 단위**라 우리 단일 용도 구조로는 재현이 안 된다.
    # → 그래서 '곱해서 흉내내지 않는다'를 params에 못박고 여기서 지킨다.
    check("별표2 주7 '용도별 보정계수'의 출처(운영규정 별표2)를 params가 담고 있음",
          "용도별보정계수_제2호" in _yaml and "운영규정" in _yaml
          and "schlPubRulSeq=2200000087233" in _yaml)
    check("어린이집이 23개 용도프로필에 없다는 사실이 기록됨",
          "'어린이집'도 '노유자시설'도 없다" in _yaml)
    check("보정계수를 우리 숫자에 곱해 흉내내지 않는다고 못박음",
          "곱해서 흉내내지 않는다" in _yaml)
    # ECO2는 ISO 13790+DIN V 18599다. 고시의 'ISO 52016'은 13790 폐지에 따른 문구 정비일 뿐
    # 엔진 변경이 아니다 — 이걸 혼동하면 엉뚱한 엔진(ISO 52016 구현체)을 붙이게 된다.
    check("ECO2 계산법이 매뉴얼 원문(ISO 13790 + DIN V 18599)으로 기록됨",
