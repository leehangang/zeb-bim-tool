...
def render_evidence_panel() -> None:
    ...
    with tab4:
        ...
        st.success(
            "🔧 **2026-07 해결** — 01 GR 가이드라인 · 02 GR 기술요소 · 09 영유아보육법은 "
            "\"이미지 스캔본이라 추출 불가\"로 색인에서 빠져 있었으나, 실제로는 페이지마다 "
            "텍스트가 동봉된 **번들 포맷**이었습니다. 인덱서가 `.jpeg`만 보고 \"텍스트 없음\"이라 "
            "단정해 버리고 있었을 뿐입니다 — **+114쪽 · 13만자 회수** (974 → 1,140청크). "
            "03은 12(ZEB 인증규칙)와 **동일 문서**라 중복 제외했습니다.",
            icon="🔧",
        )
        st.warning(
            "⚠️ **아직 없는 원문 — ZEB 인증기준 공동고시** (국토부 제2024-893호 / "
            "산업부 제2024-208호). 별표1(자립률 산식·**대지외 보정계수**)·별표2(등급표)·"
            "별표1의2(BEMS 13항목)가 여기 있습니다. 인증규칙 §8③이 "
            "*\"국토교통부장관과 기후에너지환경부장관이 정하여 공동으로 고시한다\"* 고 "
            "위임하기 때문입니다. 확보하면 위 ② 확인 필요의 **보정계수**가 "
            "'원문대조'로 승격됩니다.",
            icon="⚠️",
        )
