...
def render_evidence_panel() -> None:
    ...
    with tab2:
        ...
        st.warning(
            ...
            "환산하는 것이 다음 작업입니다. 그때까지 이 숫자는 **가정치** 입니다.",
            icon="⚠️",
        )
    ...
    with tab4:
        ...
        st.success(
            "✅ **2026-07-16 — ZEB 인증기준 공동고시 확보** (국토부 제2024-893호 / "
            "산업부 제2024-208호). 별표1(자립률 산식·**대지외 보정계수**)·별표1의2(BEMS 13항목)·"
            "별표2(등급표)·별표3·별표4를 국가법령정보센터에서 받아 색인했습니다. "
            "대조 결과 **보정계수·등급표·수수료·주거 판정이 전부 원문과 일치**해 "
            "'확인 필요' → **'원문대조'**로 승격했습니다. "
            "이 고시 링크는 팀 자료(ZEB GR 법령.pdf)에 처음부터 있었는데, 우리가 안 읽고 "
            "\"못 구했다\"고 적어두고 있었습니다.",
            icon="✅",
        )
        st.warning(
            "⚠️ **원문을 읽고 새로 생긴 숙제 2건** — ① 별표2 **주7**은 제2호 등급을 "
            "*\"연간 단위면적당 1차에너지소요량에서 운영세칙으로 정하는 **용도별 보정계수**를 "
            "반영한 결과\"* 로 재라고 합니다. 그 계수는 고시가 아니라 운영기관 **운영세칙**에 "
            "있어 아직 없습니다 — 위 등급은 **보정계수 미반영 값**입니다. "
            "② 부칙 제2020-574호 제2조가 수수료 환불을 **2024.12.31 신청분까지**로 한정하는데 "
            "현행 부칙에 연장 조항이 안 보입니다 — 도담 환불 78만원은 '확인 필요'입니다.",
            icon="⚠️",
        )
        st.caption(
            "색인된 원문의 역할별 전체 목록은 사이드바 **📚 색인 원문**에 있습니다. "
            "상세 아키텍처: `docs/ARCHITECTURE.md` · 파라미터: `data/params/*.yaml`"
        )
    ...
    with tab5:
        ...
        st.error(
            "🔴 **업로드한 BIM JSON은 서버로 전송됩니다.** 이 앱은 Streamlit Community Cloud"
            "(해외 호스팅)에서 Python으로 실행되며, 파일 파싱·진단·ROI 계산이 **전부 그 서버에서** "
            "일어납니다. 브라우저에서 처리되는 것이 아닙니다.\n\n"
            "**정책 Q&A 질문은 Anthropic API로 전송됩니다.** 검색된 법령 조항과 질문이 "
            "함께 LLM에 전달됩니다.\n\n"
            "**실제 공공건축물 도면·실측 데이터를 올리기 전에 소속 기관의 정보보호 정책을 "
            "확인하세요.** 민감한 실물 데이터는 로컬 실행(`streamlit run streamlit_app.py`)을 "
            "권합니다 — 저장소를 받아 직접 돌리면 데이터가 기관 밖으로 나가지 않습니다.",
            icon="🔴",
        )
        st.markdown("#### 저장되는 것 / 저장되지 않는 것")
        import pandas as pd
        st.dataframe(pd.DataFrame([
            {"항목": "업로드한 BIM JSON", "처리": "서버 메모리에서 파싱",
             "저장": "❌ 세션 종료 시 소멸 (DB 없음)"},
            {"항목": "진단 결과", "처리": "세션 캐시(`st.cache_data`)",
             "저장": "❌ 세션 한정 · 계정 없음"},
            {"항목": "정책 Q&A 질문", "처리": "Anthropic API 전송",
             "저장": "❌ 우리 쪽 미저장 (Anthropic 정책은 별도)"},
            {"항목": "법령 원문 색인", "처리": "저장소에 동봉된 ChromaDB",
             "저장": "✅ 공개 법령이라 민감정보 아님"},
            {"항목": "계정·로그인", "처리": "없음",
             "저장": "— 애초에 수집하지 않음"},
