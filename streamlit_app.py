...
def render_home():
    ...
    z1.metric("현재 ZEB", "인증 미달", "1차E 소요량 151 kWh/㎡·년")
    z2.metric("보강 후 ZEB", "4등급", "제2호 기준 충족 ✅")
    z3.metric("1차에너지소요량", "50.6", "151 → 50.6 kWh/㎡·년 (90 미만)")
    st.caption(
        "ZEB 인증등급은 **제1호(에너지자립률) 또는 제2호(1차에너지소요량) 중 상위 등급**으로 정하고, "
        "제3호(BEMS 13항목)를 함께 충족해야 합니다(인증기준 고시 별표2 주1·주2). "
        "**보강 후 4등급은 제2호(소요량 50.6 < 비주거 4등급 기준 90) 근거**이며, "
        "제1호(자립률)로는 5등급에 해당합니다. "
        "‘보강 후’는 11개 GR 기술요소 전체 적용 + BEMS 설치를 가정한 잠재 등급입니다."
    )
    st.warning(
        "⚠️ **확인 필요** — 현재 BIM 샘플은 도담의 **태양열집열판(27㎡, 급탕용)을 태양광 5.4kW로 분류**하고 있어 "
        "에너지자립률 수치(9.3%·23.4%)가 과대 추정입니다. 실제 PV 객체는 0개입니다. "
        "태양광/태양열 분리 후 재산출 예정이며, **등급 결론(4등급)은 제2호 근거라 영향받지 않습니다.**",
        icon="⚠️",
    )
