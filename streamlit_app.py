...
def render_home():
    ...
    st.error(
        "🔴 **4등급은 산정 방식의 산물입니다 — 결합 방식을 바로잡으면 5등급으로 내려갑니다.** "
        "현재 엔진은 11개 요소의 절감률을 **단순 덧셈**해 67%를 얻습니다. 그런데 단순합산은 "
        "물리적으로 성립하지 않습니다(10% 요소 11개면 110% → 에너지가 음수). 각 요소가 *남은* "
        "에너지를 줄인다고 보는 **1 − Π(1−rᵢ)** 로 계산하면 **50.5%** 이고, 4등급 임계 55%에 "
        "못 미쳐 **소요량 99.1 → 5등급**이 됩니다. "
        "게다가 요소별 절감률(외벽단열 15% 등) 자체도 **문헌 근거를 아직 찾지 못했습니다.**",
        icon="🔴",
    )
    st.warning(
        "⚠️ **실측 연구가 우리 방향의 과대추정을 뒷받침합니다** — 에너지경제연구원 기본연구보고서 "
        "2025-14(2026-06)는 GR 시행 공공건축물 **522동(어린이집 358동 = 68.6%)** 실측 결과 "
        "연간 **20.4 kWh/㎡** 절감으로, 엔지니어링 사전 예측 **33 kWh/㎡의 약 60% 수준**이라고 "
        "보고했습니다. 표본의 2/3이 어린이집이라 도담 케이스에 전이 가능성이 높습니다. "
        "**팀 합의 전이라 엔진 기본값은 아직 단순합산(67%)이며**, 네 가지 산정 방식과 각각의 "
        "등급 결과를 **[📐 근거·출처] → ③ 등급 민감도**에서 전부 공개합니다.",
        icon="⚠️",
    )
